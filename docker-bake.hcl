...
COPY --from=extracted /app/dependencies/ ${SPRING_BOOT_BAKE_APPDIR}/dependencies/
COPY --from=extracted /app/snapshot-dependencies/ ${SPRING_BOOT_BAKE_APPDIR}/snapshot-dependencies/
COPY --from=extracted /app/spring-boot-loader/ ${SPRING_BOOT_BAKE_APPDIR}/spring-boot-loader/
COPY --from=extracted /app/application/ ${SPRING_BOOT_BAKE_APPDIR}/application/
